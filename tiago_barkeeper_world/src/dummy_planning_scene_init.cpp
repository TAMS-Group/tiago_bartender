...
public:
  ...
  void init_scene()
  {
    ...
    shapes::Mesh* m = shapes::createMeshFromResource("package://tiago_barkeeper_world/meshes/bottle_2_coke.dae");
    ...
    glass_scale[1] = 0.001;
    glass_scale[2] = 0.001;
    m = shapes::createMeshFromResource("package://tiago_barkeeper_world/meshes/ikea_glass_binary.stl", glass_scale);
    shapes::constructMsgFromShape(m, mesh_msg);
    mesh = boost::get<shape_msgs::Mesh>(mesh_msg);
    ...
    for(auto pose : bar_poses_)
    {
      ...
      {
        m = shapes::createMeshFromResource("package://tiago_barkeeper_world/meshes/bartresen_binary.stl", bar_scale);
      }
      else if(pose.first == "cupboard")
      {
        m = shapes::createMeshFromResource("package://tiago_barkeeper_world/meshes/cupboard_binary.stl", bar_scale);
      }
      else
      {
        continue;
