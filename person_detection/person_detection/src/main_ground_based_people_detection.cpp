 ...
 *
 * TODO: 
 * make hardcoded image subscription topic a parameter
 * check hardcoded filenames (/usr/share/opencv...) for the Haar cascade configs
 *
 * (C) 2014 Sebastian Starke
 * (C) 2014 Norman Hendrich
 *
 * 13.06.2018 - move Face messages to person_detection_msgs, update here
...
#include <person_detection_msgs/FaceVector.h>
...
void publishFaces()
{
    person_detection_msgs::FaceVector faceVector;

    for(unsigned i=0; i<faces.size(); i++)
    {
        if(faces.at(i).count >= minCount)
        {
            person_detection_msgs::Face face;
...
int main(int argc, char** argv)
{
  ...
  image_transport::Subscriber image_sub_ = it_.subscribe( "camera/rgb/image_color", 1, &imageCallback);
    ...
    std::string fn_haar_frontalface_default = 
      "/usr/share/opencv/haarcascades/haarcascade_frontalface_default.xml";
      // "/opt/ros/hydro/share/OpenCV/haarcascades/haarcascade_frontalface_default.xml";
    std::string haarcascade_mcs_mouth = 
      "/usr/share/opencv/haarcascades/haarcascade_mcs_mouth.xml";
      // "/opt/ros/hydro/share/OpenCV/haarcascades/haarcascade_mcs_mouth.xml";
  ...
  pubFaces = nh.advertise<person_detection_msgs::FaceVector>( "faces", 1);
